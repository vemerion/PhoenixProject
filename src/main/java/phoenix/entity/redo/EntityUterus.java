...
import phoenix.util.EntityUtil;
...
public class EntityUterus extends EntityMob implements IEntityMultiPart
{
    ...
    protected void initEntityAI()
    {
        ...
        tasks.addTask(1, new AI.EntityAIUterusAttack(this));
    }
    ...
    static class AIMoveRandom extends EntityAIBase
    {
        ...
        public AIMoveRandom(EntityUterus uterusIn)
        {
            this.uterus = uterusIn;
        }
        ...
        public void updateTask()
        {
            ...
            int i = this.uterus.getIdleTime();

            if (i > 20)
            {
                this.uterus.setMovementVector(0.0F, 0.0F, 0.0F);
            }
            else if (this.uterus.getRNG().nextInt(50) == 0 || !this.uterus.hasMovementVector())
            {
                float f = this.uterus.getRNG().nextFloat() * ((float)Math.PI * 2F);
                float f1 = MathHelper.cos(f) * 0.2F;
                float f2 = -0.1F + this.uterus.getRNG().nextFloat() * 0.2F;
                float f3 = MathHelper.sin(f) * 0.2F;
                this.uterus.setMovementVector(f1, f2, f3);
            }
        }
    }
