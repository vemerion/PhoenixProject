...
object PhoenixItems
{
    ...
    val HIGH_QUALITY_CLAY_ITEM         = ITEMS.register("high_quality_clay", basicItem(Item.Properties().maxStackSize(16).group(Phoenix.ASH)))!!
    ...
    val ZIRCONIUM_AXE                  = ITEMS.register("ceramic_zirconium_axe")     { AxeItem    (PhoenixTiers.ZIRCONIUM_TIER, 9.0f, -3.2f, Item.Properties().group(Phoenix.ASH)) }!!
    val ZIRCONIUM_PICKAXE              = ITEMS.register("ceramic_zirconium_pickaxe") { PickaxeItem(PhoenixTiers.ZIRCONIUM_TIER, 0, -1f, Item.Properties().group(Phoenix.ASH)) }!!
    val ZIRCONIUM_SWORD                = ITEMS.register("ceramic_zirconium_sword")   { SwordItem  (PhoenixTiers.ZIRCONIUM_TIER, 2, -0.5f, Item.Properties().group(Phoenix.ASH)) }!!
    val ZIRCONIUM_KNIFE                = ITEMS.register("ceramic_zirconium_knife")   { KnifeItem  (PhoenixTiers.ZIRCONIUM_TIER, 3f, -10f, PhoenixConfiguration.COMMON_CONFIG.gameMode.get().maxKnifeUsages, Phoenix.ASH) }!!

    val STEEL_AXE                      = ITEMS.register("steel_axe")     { AxeItem    (PhoenixTiers.STEEL_TIER, 5.0f, -2f, Item.Properties().group(Phoenix.ASH)) }!!
    val STEEL_PICKAXE                  = ITEMS.register("steel_pickaxe") { PickaxeItem(PhoenixTiers.STEEL_TIER, 0, -2f, Item.Properties().group(Phoenix.ASH)) }!!
    val STEEL_SWORD                    = ITEMS.register("steel_sword")   { SwordItem  (PhoenixTiers.STEEL_TIER, 2, -2f, Item.Properties().group(Phoenix.ASH)) }!!

    val STEEL_ARMOR_HEAD               = ITEMS.register("steel_armor_head")      { SteelArmorItem(EquipmentSlotType.HEAD, Item.Properties().group(Phoenix.ASH)) }!!
    val STEEL_ARMOR_CHES               = ITEMS.register("steel_armor_chestplate"){ SteelArmorItem(EquipmentSlotType.CHEST,Item.Properties().group(Phoenix.ASH)) }!!
    val STEEL_ARMOR_LEGG               = ITEMS.register("steel_armor_leggings")  { SteelArmorItem(EquipmentSlotType.LEGS, Item.Properties().group(Phoenix.ASH)) }!!
    val STEEL_ARMOR_BUTS               = ITEMS.register("steel_armor_boots")     { SteelArmorItem(EquipmentSlotType.FEET, Item.Properties().group(Phoenix.ASH)) }!!

    @JvmStatic fun register() = ITEMS.register(thedarkcolour.kotlinforforge.forge.MOD_CONTEXT.getKEventBus())

    private fun basicItem() =  Supplier { Item(Item.Properties().group(Phoenix.ASH)) }
    private fun basicItem(prop : Item.Properties) =  Supplier { Item(prop) }
    private fun basicFood(food: Food) =  Supplier { Item(Item.Properties().group(Phoenix.ASH).food(food)) }
    private fun basicFood(food: Food, group : ItemGroup) =  Supplier { Item(Item.Properties().group(group).food(food)) }
    private fun form(contains: RegistryObject<Item>) = Supplier { Item(Item.Properties().group(Phoenix.ASH).containerItem(contains.get())) }
}
