...
import phoenix.client.render.entity.LayerPilons;
...
public class MyHooks
{
    ...
    public static void randomDisplayTick(BlockEndRod ber, IBlockState stateIn, World worldIn, BlockPos pos, Random rand)
    {
        ...
        if(worldIn.getBlockState(pos.down().down()).getBlock() == Blocks.END_ROD)
            ...
            worldIn.spawnParticle(EnumParticleTypes.END_ROD, d0 + (double) enumfacing.getXOffset() * d3, d1 + (double)enumfacing.getYOffset() * d3, d2 + (double)enumfacing.getZOffset() * d3,
                    rand.nextGaussian() * 0.005D, rand.nextGaussian() *  0.005D, rand.nextGaussian() * 0.005D, 216, 191, 216, 600 + rand.nextInt(300));
    }

    @SideOnly(Side.CLIENT)
    @Hook(targetMethod= "<init>", at = @At(point = InjectionPoint.RETURN))
    public static void  init(RenderPlayer renderPlayer, RenderManager renderManager, boolean useSmallArms)
    {
        renderPlayer.addLayer(new LayerPilons(renderPlayer));
    }
}
