...
class KnifeItem(tier: IItemTier, attackDamageIn: Float, attackSpeedIn: Float, maxUsages: Int, group : ItemGroup) : ToolItem(attackDamageIn, attackSpeedIn, tier, breakableBlocks, Properties().group(group).maxDamage(maxUsages))
{
    ...
    override fun onItemRightClick(world: World, player: PlayerEntity, hand: Hand): ActionResult<ItemStack>
    {
        ...
        if (!world.isRemote)
        {
            ...
            knife.knife = stack
            knife.func_234612_a_(player, player.rotationPitch, player.rotationYaw, 0.0f, 2f, 0.3f)
            ...
            for (i in 1..count)
            {
                addTask(10 * i)
                {
                    ...
                    knife2.func_234612_a_(player, player.rotationPitch, player.rotationYaw, 0.0f, 2.5f, 1.0f)
    ...
    fun onHitEntity(world : World, owner : LivingEntity?, knife : KnifeEntity, hitted : Entity, knifeItem : ItemStack): Boolean
    {
        ...
        hitted.attackEntityFrom(DamageSource.causeThrownDamage(knife, knife.func_234616_v_()), damage.toFloat())
    ...
    companion object
    {
        ...
        var breakableBlocksTypes  = ImmutableSet.of(Tags.Blocks.GLASS, Tags.Blocks.STAINED_GLASS_PANES)
