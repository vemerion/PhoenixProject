...
class DiaryBook(val xSize: Int, val ySizeIn: Int, renderer: FontRenderer)
{
    ...
    fun render(stack : MatrixStack, gui: ContainerScreen<DiaryContainer>, renderer: FontRenderer, xSize: Int, ySize: Int, x: Int, y: Int, depth: Int)
    {
        if(openedChapters.size < currentChapter)
        {
            ...
            for (element in page)
            {
                element.render(stack, gui, renderer, xSize, ySize, x, y + sum * (font.FONT_HEIGHT + 2), depth)
            ...
            for (element in page)
            {
                element.render(stack, gui, renderer, xSize, ySize, x + xSize / 2 - 10, y + sum * (font.FONT_HEIGHT + 2), depth)
                sum += element.getHeight(xSize, ySize)
            }
        }
    }
