...
class OvenTile : PhoenixTile<OvenTile>(PhoenixTiles.OVEN), ITickableTileEntity, IInventory
{
    ...
    fun outOtherItems() : List<ItemStack>
    {
        ...
        for (i in 0..3)
        {
            if(!recipesFromInputs.containsKey(inventory[i].item) || inventory[i].item == PhoenixItems.COOKED_SETA)
            {
    ...
    fun getOtherItems() : List<ItemStack>
    {
        ...
        for (i in 0..3)
        {
            if(!recipesFromInputs.containsKey(inventory[i].item))
            {
                res.add(inventory[i]);
            }
        }
            ...
            if(burnTime > 0)
            {
                ...
                if(world!!.getBlockState(pos)[OvenBlock.STATE] == 2)
                {
                    ...
                    for (i in 0..3)
                    {
                        ...
                        if (recipesFromInputs.containsKey(current.item))
                        {
                    ...
                    if (has)
                        NetworkHandler.sendToDim(SyncOvenPacket(this), world!!.dimensionKey)
                }
                if (burnTime == 0)
