...
object PhoenixItems
{
    val ITEMS = KDeferredRegister(ForgeRegistries.ITEMS, Phoenix.MOD_ID)

    //val GUIDE                          = ITEMS.register("diary", ::ItemDiary)!!

    val HIGH_QUALITY_CLAY_ITEM         by ITEMS.register("high_quality_clay", basicItem(Item.Properties().maxStackSize(16).group(Phoenix.ASH)))
    val COOKED_SETA                    by ITEMS.register("cooked_seta", basicFood(Food.Builder().hunger(6).saturation(3f).fastToEat().build(), ItemGroup.FOOD))
    val GOLDEN_SETA                    by ITEMS.register("golden_seta", basicFood(Food.Builder().hunger(7).saturation(4f).fastToEat().build(), ItemGroup.FOOD))
    ...
    val STEEL_SWORD                    by ITEMS.register("steel_sword")   { SwordItem  (PhoenixTiers.STEEL_TIER, 2, -2f, Item.Properties().group(Phoenix.ASH)) }
