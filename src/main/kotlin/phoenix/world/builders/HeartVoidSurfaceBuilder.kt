...
class HeartVoidSurfaceBuilder(function: Codec<SurfaceBuilderConfig>) : SurfaceBuilder<SurfaceBuilderConfig>(function)
{
        ...
        for (y in startHeight downTo 0)
        {
            ...
            } else if (currectState.block === defaultBlock.block)
            {
                                ...
                                isAir(chunkIn, currect_pos.west(9)) || isAir(chunkIn, currect_pos.east(9))))
                {
                    ...
                    {
                        chunkIn.setBlockState(currect_pos, PhoenixBlocks.ANTI_AIR.defaultState, false)
                    }
                } else if (i == -1)
                {
                    if (noiseHeight <= 0)
