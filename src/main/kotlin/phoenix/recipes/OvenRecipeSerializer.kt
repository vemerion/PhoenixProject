...
{
    private val cookingTime = 100

    @Nonnull
    override fun read(recipeId: ResourceLocation, json: JsonObject): OvenRecipe
    {
        val s = JSONUtils.getString(json, "group", "")
        val jsonelement = if (JSONUtils.isJsonArray(json, "ingredient")) JSONUtils.getJsonArray(
            json,
            "ingredient"
        ) else JSONUtils.getJsonObject(json, "ingredient")
        ...
        itemstack =
            if (json["result"].isJsonObject) ShapedRecipe.deserializeItem(JSONUtils.getJsonObject(json, "result")) else
            {
                ...
                ItemStack(Registry.ITEM.getOptional(resourcelocation).orElseThrow {
    ...
    override fun read(recipeId: ResourceLocation, buffer: PacketBuffer): OvenRecipe
    {
        ...
        var cookTime = buffer.readVarInt()
        cookTime = if (cookTime as Any == null) 40 else cookTime
    ...
    interface IFactory<T : AbstractCookingRecipe?>
    {
        fun create(
            id: ResourceLocation?,
            group: String?,
